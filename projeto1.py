...
async def retornar_enderecos_do_usuario(id_usuario: int, id_endereco: int):
    ...
    # for u in db_usuarios:
    #     for e in db_end:
    #         if u.id == id_usuario and e.id == id_endereco:
...
@app.get("/endereco/{id_endereco}/")
async def retornar_endereco(id_endereco: int):
        for e in db_end:
            if e.id == id_endereco:
                return e
        return FALHA
...
async def retornar_produto_com_nome(id_produto: int):
